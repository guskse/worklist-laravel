    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <section class="md:col-span-3">
            <div class="rounded-lg shadow-md bg-white p-3">
                <div class="flex justify-between items-center">
                    ...
                    {{-- only show the edit button and delete form if user is the creator of the job listing --}}
                    @can('update', $job)
                    <!--Edit Button -->
                ...
                <div class="p-4">
                    ...
                    <ul class="my-4 bg-gray-100 p-4">
                        ...
                        <li class="mb-2">
                            <strong>Tags:</strong>
                            @if($job->tags)
                            {{-- change the words to uppercase and replace the commas with space and comma --}}
            ...
            <div class="container mx-auto p-4">
                ...
                @auth
                {{-- APPLY BUTTON with aplineJS--}}
                <div x-data="{ open: false }">
                    <button @click="open = true" class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium cursor-pointer text-indigo-700 bg-indigo-100 hover:bg-indigo-200">Apply Now
                    </button>

                    {{-- FORM MODAL --}}
                    <div x-cloak x-show="open" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">

                        <div @click.away="open = false" class="max-h-96 bg-white overflow-y-auto p-6 rounded-lg shadow-md w-full max-w-md">
        ...
        <aside class="bg-white rounded-lg shadow-md p-3">
            ...
            <form method="POST" action={{auth()->user()->bookmarkedJobs()->where('job_id', $job->id)->exists() ? route('bookmarks.destroy', $job->id) : route('bookmarks.store', $job->id) }} class="mt-10">
                ...
                {{-- Check if job is already bookmarked, and show the buttons accordingly --}}
                @if(auth()->user()->bookmarkedJobs()->where('job_id', $job->id)->exists())
                @method("DELETE")
                <button class="bg-red-500 text-white font-bold w-full py-2 px-4 rounded-full flex items-center justify-center hover:bg-red-600">
                    <i class="fas fa-bookmark mr-3"></i>Remove bookmark
                </button>

                @else
                <button class="bg-blue-500 text-white font-bold w-full py-2 px-4 rounded-full flex items-center justify-center hover:bg-blue-600">
